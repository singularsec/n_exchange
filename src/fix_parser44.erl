...
decode_message([{msg_type,ioi}|Message]) -> % IOI
  decode_fields(Message, #ioi{}, ioi, 23);

decode_message([{msg_type,advertisement}|Message]) -> % Advertisement
  decode_fields(Message, #advertisement{}, advertisement, 21);
...
decode_message([{msg_type,news}|Message]) -> % News
  decode_fields(Message, #news{}, news, 14);

decode_message([{msg_type,email}|Message]) -> % Email
  decode_fields(Message, #email{}, email, 16);
...
decode_message([{msg_type,new_order_list}|Message]) -> % NewOrderList
  decode_fields(Message, #new_order_list{}, new_order_list, 71);
...
decode_message([{msg_type,order_status_request}|Message]) -> % OrderStatusRequest
  decode_fields(Message, #order_status_request{}, order_status_request, 12);

decode_message([{msg_type,allocation_instruction}|Message]) -> % AllocationInstruction
  decode_fields(Message, #allocation_instruction{}, allocation_instruction, 61);

decode_message([{msg_type,list_cancel_request}|Message]) -> % ListCancelRequest
  decode_fields(Message, #list_cancel_request{}, list_cancel_request, 9);

decode_message([{msg_type,list_execute}|Message]) -> % ListExecute
  decode_fields(Message, #list_execute{}, list_execute, 9);

decode_message([{msg_type,list_status_request}|Message]) -> % ListStatusRequest
  decode_fields(Message, #list_status_request{}, list_status_request, 6);

decode_message([{msg_type,list_status}|Message]) -> % ListStatus
  decode_fields(Message, #list_status{}, list_status, 14);

decode_message([{msg_type,allocation_instruction_ack}|Message]) -> % AllocationInstructionAck
  decode_fields(Message, #allocation_instruction_ack{}, allocation_instruction_ack, 17);

decode_message([{msg_type,dont_know_trade}|Message]) -> % DontKnowTrade
  decode_fields(Message, #dont_know_trade{}, dont_know_trade, 14);
...
decode_message([{msg_type,quote}|Message]) -> % Quote
  decode_fields(Message, #quote{}, quote, 54);

decode_message([{msg_type,settlement_instructions}|Message]) -> % SettlementInstructions
  decode_fields(Message, #settlement_instructions{}, settlement_instructions, 12);
...
decode_message([{msg_type,market_data_snapshot_full_refresh}|Message]) -> % MarketDataSnapshotFullRefresh
  decode_fields(Message, #market_data_snapshot_full_refresh{}, market_data_snapshot_full_refresh, 12);
...
decode_message([{msg_type,quote_cancel}|Message]) -> % QuoteCancel
  decode_fields(Message, #quote_cancel{}, quote_cancel, 14);

decode_message([{msg_type,quote_status_request}|Message]) -> % QuoteStatusRequest
  decode_fields(Message, #quote_status_request{}, quote_status_request, 13);

decode_message([{msg_type,mass_quote_acknowledgement}|Message]) -> % MassQuoteAcknowledgement
  decode_fields(Message, #mass_quote_acknowledgement{}, mass_quote_acknowledgement, 39);

decode_message([{msg_type,security_definition_request}|Message]) -> % SecurityDefinitionRequest
  decode_fields(Message, #security_definition_request{}, security_definition_request, 14);

decode_message([{msg_type,security_definition}|Message]) -> % SecurityDefinition
  decode_fields(Message, #security_definition{}, security_definition, 16);

decode_message([{msg_type,security_status_request}|Message]) -> % SecurityStatusRequest
  decode_fields(Message, #security_status_request{}, security_status_request, 10);

decode_message([{msg_type,security_status}|Message]) -> % SecurityStatus
  decode_fields(Message, #security_status{}, security_status, 25);

decode_message([{msg_type,trading_session_status_request}|Message]) -> % TradingSessionStatusRequest
  decode_fields(Message, #trading_session_status_request{}, trading_session_status_request, 9);

decode_message([{msg_type,trading_session_status}|Message]) -> % TradingSessionStatus
  decode_fields(Message, #trading_session_status{}, trading_session_status, 19);

decode_message([{msg_type,mass_quote}|Message]) -> % MassQuote
  decode_fields(Message, #mass_quote{}, mass_quote, 36);
...
decode_message([{msg_type,rfq_request}|Message]) -> % RFQRequest
  decode_fields(Message, #rfq_request{}, rfq_request, 12);
...
decode_message([{msg_type,quote_response}|Message]) -> % QuoteResponse
  decode_fields(Message, #quote_response{}, quote_response, 56);
...
field_index(ioi, sending_time) -> 2;
field_index(ioi, ioi_id) -> 3;
field_index(ioi, ioi_trans_type) -> 4;
field_index(ioi, ioi_ref_id) -> 5;
field_index(ioi, side) -> 6;
field_index(ioi, qty_type) -> 7;
field_index(ioi, ioi_qty) -> 8;
field_index(ioi, currency) -> 9;
field_index(ioi, price_type) -> 10;
field_index(ioi, price) -> 11;
field_index(ioi, valid_until_time) -> 12;
field_index(ioi, ioi_qlty_ind) -> 13;
field_index(ioi, ioi_natural_flag) -> 14;
field_index(ioi, text) -> 15;
field_index(ioi, encoded_text) -> 16;
field_index(ioi, transact_time) -> 17;
field_index(ioi, url_link) -> 18;
field_index(advertisement, sending_time) -> 2;
field_index(advertisement, adv_id) -> 3;
field_index(advertisement, adv_trans_type) -> 4;
field_index(advertisement, adv_ref_id) -> 5;
field_index(advertisement, adv_side) -> 6;
field_index(advertisement, quantity) -> 7;
field_index(advertisement, qty_type) -> 8;
field_index(advertisement, price) -> 9;
field_index(advertisement, currency) -> 10;
field_index(advertisement, trade_date) -> 11;
field_index(advertisement, transact_time) -> 12;
field_index(advertisement, text) -> 13;
field_index(advertisement, encoded_text) -> 14;
field_index(advertisement, url_link) -> 15;
field_index(advertisement, last_mkt) -> 16;
field_index(advertisement, trading_session_id) -> 17;
field_index(advertisement, trading_session_sub_id) -> 18;
...
field_index(news, sending_time) -> 2;
field_index(news, orig_time) -> 3;
field_index(news, urgency) -> 4;
field_index(news, headline) -> 5;
field_index(news, encoded_headline) -> 6;
field_index(news, url_link) -> 7;
field_index(news, raw_data) -> 8;
field_index(email, sending_time) -> 2;
field_index(email, email_thread_id) -> 3;
field_index(email, email_type) -> 4;
field_index(email, orig_time) -> 5;
field_index(email, subject) -> 6;
field_index(email, encoded_subject) -> 7;
field_index(email, order_id) -> 8;
field_index(email, cl_ord_id) -> 9;
field_index(email, raw_data) -> 10;
...
field_index(new_order_list, sending_time) -> 2;
field_index(new_order_list, list_id) -> 3;
field_index(new_order_list, bid_id) -> 4;
field_index(new_order_list, client_bid_id) -> 5;
field_index(new_order_list, prog_rpt_reqs) -> 6;
field_index(new_order_list, bid_type) -> 7;
field_index(new_order_list, prog_period_interval) -> 8;
field_index(new_order_list, cancellation_rights) -> 9;
field_index(new_order_list, money_laundering_status) -> 10;
field_index(new_order_list, regist_id) -> 11;
field_index(new_order_list, list_exec_inst_type) -> 12;
field_index(new_order_list, list_exec_inst) -> 13;
field_index(new_order_list, encoded_list_exec_inst) -> 14;
field_index(new_order_list, allowable_one_sidedness_pct) -> 15;
field_index(new_order_list, allowable_one_sidedness_value) -> 16;
field_index(new_order_list, allowable_one_sidedness_curr) -> 17;
field_index(new_order_list, tot_no_orders) -> 18;
field_index(new_order_list, last_fragment) -> 19;
field_index(new_order_list, settl_type) -> 20;
field_index(new_order_list, settl_date) -> 21;
field_index(new_order_list, cash_margin) -> 22;
field_index(new_order_list, clearing_fee_indicator) -> 23;
field_index(new_order_list, handl_inst) -> 24;
field_index(new_order_list, exec_inst) -> 25;
field_index(new_order_list, min_qty) -> 26;
field_index(new_order_list, max_floor) -> 27;
field_index(new_order_list, ex_destination) -> 28;
field_index(new_order_list, process_code) -> 29;
field_index(new_order_list, prev_close_px) -> 30;
field_index(new_order_list, side) -> 31;
field_index(new_order_list, side_value_ind) -> 32;
field_index(new_order_list, locate_reqd) -> 33;
field_index(new_order_list, transact_time) -> 34;
field_index(new_order_list, qty_type) -> 35;
field_index(new_order_list, ord_type) -> 36;
field_index(new_order_list, price_type) -> 37;
field_index(new_order_list, price) -> 38;
field_index(new_order_list, stop_px) -> 39;
field_index(new_order_list, currency) -> 40;
field_index(new_order_list, compliance_id) -> 41;
field_index(new_order_list, solicited_flag) -> 42;
field_index(new_order_list, ioi_id) -> 43;
field_index(new_order_list, quote_id) -> 44;
field_index(new_order_list, time_in_force) -> 45;
field_index(new_order_list, effective_time) -> 46;
field_index(new_order_list, expire_date) -> 47;
field_index(new_order_list, expire_time) -> 48;
field_index(new_order_list, gt_booking_inst) -> 49;
field_index(new_order_list, order_capacity) -> 50;
field_index(new_order_list, order_restrictions) -> 51;
field_index(new_order_list, cust_order_capacity) -> 52;
field_index(new_order_list, forex_req) -> 53;
field_index(new_order_list, settl_currency) -> 54;
field_index(new_order_list, booking_type) -> 55;
field_index(new_order_list, text) -> 56;
field_index(new_order_list, encoded_text) -> 57;
field_index(new_order_list, settl_date2) -> 58;
field_index(new_order_list, order_qty2) -> 59;
field_index(new_order_list, price2) -> 60;
field_index(new_order_list, position_effect) -> 61;
field_index(new_order_list, covered_or_uncovered) -> 62;
field_index(new_order_list, max_show) -> 63;
field_index(new_order_list, target_strategy) -> 64;
field_index(new_order_list, target_strategy_parameters) -> 65;
field_index(new_order_list, participation_rate) -> 66;
field_index(new_order_list, designation) -> 67;
...
field_index(order_status_request, order_id) -> 3;
field_index(order_status_request, cl_ord_id) -> 4;
field_index(order_status_request, secondary_cl_ord_id) -> 5;
field_index(order_status_request, cl_ord_link_id) -> 6;
field_index(order_status_request, ord_status_req_id) -> 7;
field_index(order_status_request, account) -> 8;
field_index(order_status_request, acct_id_source) -> 9;
field_index(order_status_request, side) -> 10;
field_index(allocation_instruction, sending_time) -> 2;
field_index(allocation_instruction, alloc_id) -> 3;
field_index(allocation_instruction, alloc_trans_type) -> 4;
field_index(allocation_instruction, alloc_type) -> 5;
field_index(allocation_instruction, secondary_alloc_id) -> 6;
field_index(allocation_instruction, ref_alloc_id) -> 7;
field_index(allocation_instruction, alloc_canc_replace_reason) -> 8;
field_index(allocation_instruction, alloc_intermed_req_type) -> 9;
field_index(allocation_instruction, alloc_link_id) -> 10;
field_index(allocation_instruction, alloc_link_type) -> 11;
field_index(allocation_instruction, booking_ref_id) -> 12;
field_index(allocation_instruction, alloc_no_orders_type) -> 13;
field_index(allocation_instruction, previously_reported) -> 14;
field_index(allocation_instruction, reversal_indicator) -> 15;
field_index(allocation_instruction, match_type) -> 16;
field_index(allocation_instruction, side) -> 17;
field_index(allocation_instruction, quantity) -> 18;
field_index(allocation_instruction, qty_type) -> 19;
field_index(allocation_instruction, last_mkt) -> 20;
field_index(allocation_instruction, trade_origination_date) -> 21;
field_index(allocation_instruction, trading_session_id) -> 22;
field_index(allocation_instruction, trading_session_sub_id) -> 23;
field_index(allocation_instruction, price_type) -> 24;
field_index(allocation_instruction, avg_px) -> 25;
field_index(allocation_instruction, avg_par_px) -> 26;
field_index(allocation_instruction, currency) -> 27;
field_index(allocation_instruction, avg_px_precision) -> 28;
field_index(allocation_instruction, trade_date) -> 29;
field_index(allocation_instruction, transact_time) -> 30;
field_index(allocation_instruction, settl_type) -> 31;
field_index(allocation_instruction, settl_date) -> 32;
field_index(allocation_instruction, booking_type) -> 33;
field_index(allocation_instruction, gross_trade_amt) -> 34;
field_index(allocation_instruction, concession) -> 35;
field_index(allocation_instruction, total_takedown) -> 36;
field_index(allocation_instruction, net_money) -> 37;
field_index(allocation_instruction, position_effect) -> 38;
field_index(allocation_instruction, auto_accept_indicator) -> 39;
field_index(allocation_instruction, text) -> 40;
field_index(allocation_instruction, encoded_text) -> 41;
field_index(allocation_instruction, num_days_interest) -> 42;
field_index(allocation_instruction, accrued_interest_rate) -> 43;
field_index(allocation_instruction, accrued_interest_amt) -> 44;
field_index(allocation_instruction, total_accrued_interest_amt) -> 45;
field_index(allocation_instruction, interest_at_maturity) -> 46;
field_index(allocation_instruction, end_accrued_interest_amt) -> 47;
field_index(allocation_instruction, start_cash) -> 48;
field_index(allocation_instruction, end_cash) -> 49;
field_index(allocation_instruction, legal_confirm) -> 50;
field_index(allocation_instruction, tot_no_allocs) -> 51;
field_index(allocation_instruction, last_fragment) -> 52;
field_index(allocation_instruction, clearing_fee_indicator) -> 53;
field_index(allocation_instruction, alloc_settl_inst_type) -> 54;
field_index(list_cancel_request, sending_time) -> 2;
field_index(list_cancel_request, list_id) -> 3;
field_index(list_cancel_request, transact_time) -> 4;
field_index(list_cancel_request, trade_origination_date) -> 5;
field_index(list_cancel_request, trade_date) -> 6;
field_index(list_cancel_request, text) -> 7;
field_index(list_cancel_request, encoded_text) -> 8;
field_index(list_execute, sending_time) -> 2;
field_index(list_execute, list_id) -> 3;
field_index(list_execute, client_bid_id) -> 4;
field_index(list_execute, bid_id) -> 5;
field_index(list_execute, transact_time) -> 6;
field_index(list_execute, text) -> 7;
field_index(list_execute, encoded_text) -> 8;
field_index(list_status_request, sending_time) -> 2;
field_index(list_status_request, list_id) -> 3;
field_index(list_status_request, text) -> 4;
field_index(list_status_request, encoded_text) -> 5;
field_index(list_status, sending_time) -> 2;
field_index(list_status, list_id) -> 3;
field_index(list_status, list_status_type) -> 4;
field_index(list_status, no_rpts) -> 5;
field_index(list_status, list_order_status) -> 6;
field_index(list_status, rpt_seq) -> 7;
field_index(list_status, list_status_text) -> 8;
field_index(list_status, encoded_list_status_text) -> 9;
field_index(list_status, transact_time) -> 10;
field_index(list_status, tot_no_orders) -> 11;
field_index(list_status, last_fragment) -> 12;
field_index(allocation_instruction_ack, sending_time) -> 2;
field_index(allocation_instruction_ack, alloc_id) -> 3;
field_index(allocation_instruction_ack, secondary_alloc_id) -> 4;
field_index(allocation_instruction_ack, trade_date) -> 5;
field_index(allocation_instruction_ack, transact_time) -> 6;
field_index(allocation_instruction_ack, alloc_status) -> 7;
field_index(allocation_instruction_ack, alloc_rej_code) -> 8;
field_index(allocation_instruction_ack, alloc_type) -> 9;
field_index(allocation_instruction_ack, alloc_intermed_req_type) -> 10;
field_index(allocation_instruction_ack, match_status) -> 11;
field_index(allocation_instruction_ack, product) -> 12;
field_index(allocation_instruction_ack, security_type) -> 13;
field_index(allocation_instruction_ack, text) -> 14;
field_index(allocation_instruction_ack, encoded_text) -> 15;
field_index(dont_know_trade, sending_time) -> 2;
field_index(dont_know_trade, order_id) -> 3;
field_index(dont_know_trade, secondary_order_id) -> 4;
field_index(dont_know_trade, exec_id) -> 5;
field_index(dont_know_trade, dk_reason) -> 6;
field_index(dont_know_trade, side) -> 7;
field_index(dont_know_trade, last_qty) -> 8;
field_index(dont_know_trade, last_px) -> 9;
field_index(dont_know_trade, text) -> 10;
field_index(dont_know_trade, encoded_text) -> 11;
...
field_index(quote, sending_time) -> 2;
field_index(quote, quote_req_id) -> 3;
field_index(quote, quote_id) -> 4;
field_index(quote, quote_resp_id) -> 5;
field_index(quote, quote_type) -> 6;
field_index(quote, quote_response_level) -> 7;
field_index(quote, trading_session_id) -> 8;
field_index(quote, trading_session_sub_id) -> 9;
field_index(quote, side) -> 10;
field_index(quote, settl_type) -> 11;
field_index(quote, settl_date) -> 12;
field_index(quote, settl_date2) -> 13;
field_index(quote, order_qty2) -> 14;
field_index(quote, currency) -> 15;
field_index(quote, account) -> 16;
field_index(quote, acct_id_source) -> 17;
field_index(quote, account_type) -> 18;
field_index(quote, bid_px) -> 19;
field_index(quote, offer_px) -> 20;
field_index(quote, mkt_bid_px) -> 21;
field_index(quote, mkt_offer_px) -> 22;
field_index(quote, min_bid_size) -> 23;
field_index(quote, bid_size) -> 24;
field_index(quote, min_offer_size) -> 25;
field_index(quote, offer_size) -> 26;
field_index(quote, valid_until_time) -> 27;
field_index(quote, bid_spot_rate) -> 28;
field_index(quote, offer_spot_rate) -> 29;
field_index(quote, bid_forward_points) -> 30;
field_index(quote, offer_forward_points) -> 31;
field_index(quote, mid_px) -> 32;
field_index(quote, bid_yield) -> 33;
field_index(quote, mid_yield) -> 34;
field_index(quote, offer_yield) -> 35;
field_index(quote, transact_time) -> 36;
field_index(quote, ord_type) -> 37;
field_index(quote, bid_forward_points2) -> 38;
field_index(quote, offer_forward_points2) -> 39;
field_index(quote, settl_curr_bid_fx_rate) -> 40;
field_index(quote, settl_curr_offer_fx_rate) -> 41;
field_index(quote, settl_curr_fx_rate_calc) -> 42;
field_index(quote, comm_type) -> 43;
field_index(quote, commission) -> 44;
field_index(quote, cust_order_capacity) -> 45;
field_index(quote, ex_destination) -> 46;
field_index(quote, order_capacity) -> 47;
field_index(quote, price_type) -> 48;
field_index(quote, text) -> 49;
field_index(quote, encoded_text) -> 50;
field_index(settlement_instructions, sending_time) -> 2;
field_index(settlement_instructions, settl_inst_msg_id) -> 3;
field_index(settlement_instructions, settl_inst_req_id) -> 4;
field_index(settlement_instructions, settl_inst_mode) -> 5;
field_index(settlement_instructions, settl_inst_req_rej_code) -> 6;
field_index(settlement_instructions, text) -> 7;
field_index(settlement_instructions, encoded_text) -> 8;
field_index(settlement_instructions, cl_ord_id) -> 9;
field_index(settlement_instructions, transact_time) -> 10;
...
field_index(market_data_snapshot_full_refresh, md_req_id) -> 3;
field_index(market_data_snapshot_full_refresh, financial_status) -> 4;
field_index(market_data_snapshot_full_refresh, corporate_action) -> 5;
field_index(market_data_snapshot_full_refresh, net_chg_prev_day) -> 6;
field_index(market_data_snapshot_full_refresh, appl_queue_depth) -> 7;
field_index(market_data_snapshot_full_refresh, appl_queue_resolution) -> 8;
...
field_index(quote_cancel, sending_time) -> 2;
field_index(quote_cancel, quote_req_id) -> 3;
field_index(quote_cancel, quote_id) -> 4;
field_index(quote_cancel, quote_cancel_type) -> 5;
field_index(quote_cancel, quote_response_level) -> 6;
field_index(quote_cancel, account) -> 7;
field_index(quote_cancel, acct_id_source) -> 8;
field_index(quote_cancel, account_type) -> 9;
field_index(quote_cancel, trading_session_id) -> 10;
field_index(quote_cancel, trading_session_sub_id) -> 11;
field_index(quote_status_request, sending_time) -> 2;
field_index(quote_status_request, quote_status_req_id) -> 3;
field_index(quote_status_request, quote_id) -> 4;
field_index(quote_status_request, account) -> 5;
field_index(quote_status_request, acct_id_source) -> 6;
field_index(quote_status_request, account_type) -> 7;
field_index(quote_status_request, trading_session_id) -> 8;
field_index(quote_status_request, trading_session_sub_id) -> 9;
field_index(quote_status_request, subscription_request_type) -> 10;
field_index(mass_quote_acknowledgement, sending_time) -> 2;
field_index(mass_quote_acknowledgement, quote_req_id) -> 3;
field_index(mass_quote_acknowledgement, quote_id) -> 4;
field_index(mass_quote_acknowledgement, quote_status) -> 5;
field_index(mass_quote_acknowledgement, quote_reject_reason) -> 6;
field_index(mass_quote_acknowledgement, quote_response_level) -> 7;
field_index(mass_quote_acknowledgement, quote_type) -> 8;
field_index(mass_quote_acknowledgement, account) -> 9;
field_index(mass_quote_acknowledgement, acct_id_source) -> 10;
field_index(mass_quote_acknowledgement, account_type) -> 11;
field_index(mass_quote_acknowledgement, text) -> 12;
field_index(mass_quote_acknowledgement, encoded_text) -> 13;
field_index(mass_quote_acknowledgement, bid_px) -> 14;
field_index(mass_quote_acknowledgement, offer_px) -> 15;
field_index(mass_quote_acknowledgement, bid_size) -> 16;
field_index(mass_quote_acknowledgement, offer_size) -> 17;
field_index(mass_quote_acknowledgement, valid_until_time) -> 18;
field_index(mass_quote_acknowledgement, bid_spot_rate) -> 19;
field_index(mass_quote_acknowledgement, offer_spot_rate) -> 20;
field_index(mass_quote_acknowledgement, bid_forward_points) -> 21;
field_index(mass_quote_acknowledgement, offer_forward_points) -> 22;
field_index(mass_quote_acknowledgement, mid_px) -> 23;
field_index(mass_quote_acknowledgement, bid_yield) -> 24;
field_index(mass_quote_acknowledgement, mid_yield) -> 25;
field_index(mass_quote_acknowledgement, offer_yield) -> 26;
field_index(mass_quote_acknowledgement, transact_time) -> 27;
field_index(mass_quote_acknowledgement, trading_session_id) -> 28;
field_index(mass_quote_acknowledgement, trading_session_sub_id) -> 29;
field_index(mass_quote_acknowledgement, settl_date) -> 30;
field_index(mass_quote_acknowledgement, ord_type) -> 31;
field_index(mass_quote_acknowledgement, settl_date2) -> 32;
field_index(mass_quote_acknowledgement, order_qty2) -> 33;
field_index(mass_quote_acknowledgement, bid_forward_points2) -> 34;
field_index(mass_quote_acknowledgement, offer_forward_points2) -> 35;
field_index(mass_quote_acknowledgement, currency) -> 36;
field_index(mass_quote_acknowledgement, quote_entry_reject_reason) -> 37;
field_index(security_definition_request, sending_time) -> 2;
field_index(security_definition_request, security_req_id) -> 3;
field_index(security_definition_request, security_request_type) -> 4;
field_index(security_definition_request, currency) -> 5;
field_index(security_definition_request, text) -> 6;
field_index(security_definition_request, encoded_text) -> 7;
field_index(security_definition_request, trading_session_id) -> 8;
field_index(security_definition_request, trading_session_sub_id) -> 9;
field_index(security_definition_request, expiration_cycle) -> 10;
field_index(security_definition_request, subscription_request_type) -> 11;
field_index(security_definition, sending_time) -> 2;
field_index(security_definition, security_req_id) -> 3;
field_index(security_definition, security_response_id) -> 4;
field_index(security_definition, security_response_type) -> 5;
field_index(security_definition, currency) -> 6;
field_index(security_definition, trading_session_id) -> 7;
field_index(security_definition, trading_session_sub_id) -> 8;
field_index(security_definition, text) -> 9;
field_index(security_definition, encoded_text) -> 10;
field_index(security_definition, expiration_cycle) -> 11;
field_index(security_definition, round_lot) -> 12;
field_index(security_definition, min_trade_vol) -> 13;
field_index(security_status_request, sending_time) -> 2;
field_index(security_status_request, security_status_req_id) -> 3;
field_index(security_status_request, currency) -> 4;
field_index(security_status_request, subscription_request_type) -> 5;
field_index(security_status_request, trading_session_id) -> 6;
field_index(security_status_request, trading_session_sub_id) -> 7;
field_index(security_status, sending_time) -> 2;
field_index(security_status, security_status_req_id) -> 3;
field_index(security_status, currency) -> 4;
field_index(security_status, trading_session_id) -> 5;
field_index(security_status, trading_session_sub_id) -> 6;
field_index(security_status, unsolicited_indicator) -> 7;
field_index(security_status, security_trading_status) -> 8;
field_index(security_status, financial_status) -> 9;
field_index(security_status, corporate_action) -> 10;
field_index(security_status, halt_reason_char) -> 11;
field_index(security_status, in_view_of_common) -> 12;
field_index(security_status, due_to_related) -> 13;
field_index(security_status, buy_volume) -> 14;
field_index(security_status, sell_volume) -> 15;
field_index(security_status, high_px) -> 16;
field_index(security_status, low_px) -> 17;
field_index(security_status, last_px) -> 18;
field_index(security_status, transact_time) -> 19;
field_index(security_status, adjustment) -> 20;
field_index(security_status, text) -> 21;
field_index(security_status, encoded_text) -> 22;
field_index(trading_session_status_request, sending_time) -> 2;
field_index(trading_session_status_request, trad_ses_req_id) -> 3;
field_index(trading_session_status_request, trading_session_id) -> 4;
field_index(trading_session_status_request, trading_session_sub_id) -> 5;
field_index(trading_session_status_request, trad_ses_method) -> 6;
field_index(trading_session_status_request, trad_ses_mode) -> 7;
field_index(trading_session_status_request, subscription_request_type) -> 8;
field_index(trading_session_status, sending_time) -> 2;
field_index(trading_session_status, trad_ses_req_id) -> 3;
field_index(trading_session_status, trading_session_id) -> 4;
field_index(trading_session_status, trading_session_sub_id) -> 5;
field_index(trading_session_status, trad_ses_method) -> 6;
field_index(trading_session_status, trad_ses_mode) -> 7;
field_index(trading_session_status, unsolicited_indicator) -> 8;
field_index(trading_session_status, trad_ses_status) -> 9;
field_index(trading_session_status, trad_ses_status_rej_reason) -> 10;
field_index(trading_session_status, trad_ses_start_time) -> 11;
field_index(trading_session_status, trad_ses_open_time) -> 12;
field_index(trading_session_status, trad_ses_pre_close_time) -> 13;
field_index(trading_session_status, trad_ses_close_time) -> 14;
field_index(trading_session_status, trad_ses_end_time) -> 15;
field_index(trading_session_status, total_volume_traded) -> 16;
field_index(trading_session_status, text) -> 17;
field_index(trading_session_status, encoded_text) -> 18;
field_index(mass_quote, sending_time) -> 2;
field_index(mass_quote, quote_req_id) -> 3;
field_index(mass_quote, quote_id) -> 4;
field_index(mass_quote, quote_type) -> 5;
field_index(mass_quote, quote_response_level) -> 6;
field_index(mass_quote, account) -> 7;
field_index(mass_quote, acct_id_source) -> 8;
field_index(mass_quote, account_type) -> 9;
field_index(mass_quote, def_bid_size) -> 10;
field_index(mass_quote, def_offer_size) -> 11;
field_index(mass_quote, bid_px) -> 12;
field_index(mass_quote, offer_px) -> 13;
field_index(mass_quote, bid_size) -> 14;
field_index(mass_quote, offer_size) -> 15;
field_index(mass_quote, valid_until_time) -> 16;
field_index(mass_quote, bid_spot_rate) -> 17;
field_index(mass_quote, offer_spot_rate) -> 18;
field_index(mass_quote, bid_forward_points) -> 19;
field_index(mass_quote, offer_forward_points) -> 20;
field_index(mass_quote, mid_px) -> 21;
field_index(mass_quote, bid_yield) -> 22;
field_index(mass_quote, mid_yield) -> 23;
field_index(mass_quote, offer_yield) -> 24;
field_index(mass_quote, transact_time) -> 25;
field_index(mass_quote, trading_session_id) -> 26;
field_index(mass_quote, trading_session_sub_id) -> 27;
field_index(mass_quote, settl_date) -> 28;
field_index(mass_quote, ord_type) -> 29;
field_index(mass_quote, settl_date2) -> 30;
field_index(mass_quote, order_qty2) -> 31;
field_index(mass_quote, bid_forward_points2) -> 32;
field_index(mass_quote, offer_forward_points2) -> 33;
field_index(mass_quote, currency) -> 34;
...
field_index(bid_request, sending_time) -> 2;
field_index(bid_request, bid_id) -> 3;
field_index(bid_request, client_bid_id) -> 4;
field_index(bid_request, bid_request_trans_type) -> 5;
field_index(bid_request, list_name) -> 6;
field_index(bid_request, tot_no_related_sym) -> 7;
field_index(bid_request, bid_type) -> 8;
field_index(bid_request, num_tickets) -> 9;
field_index(bid_request, currency) -> 10;
field_index(bid_request, side_value1) -> 11;
field_index(bid_request, side_value2) -> 12;
field_index(bid_request, liquidity_ind_type) -> 13;
field_index(bid_request, wt_average_liquidity) -> 14;
field_index(bid_request, exchange_for_physical) -> 15;
field_index(bid_request, out_main_cntry_u_index) -> 16;
field_index(bid_request, cross_percent) -> 17;
field_index(bid_request, prog_rpt_reqs) -> 18;
field_index(bid_request, prog_period_interval) -> 19;
field_index(bid_request, inc_tax_ind) -> 20;
field_index(bid_request, forex_req) -> 21;
field_index(bid_request, num_bidders) -> 22;
field_index(bid_request, trade_date) -> 23;
field_index(bid_request, bid_trade_type) -> 24;
field_index(bid_request, basis_px_type) -> 25;
field_index(bid_request, strike_time) -> 26;
field_index(bid_request, text) -> 27;
field_index(bid_request, encoded_text) -> 28;
field_index(bid_response, sending_time) -> 2;
field_index(bid_response, bid_id) -> 3;
field_index(bid_response, client_bid_id) -> 4;
field_index(list_strike_price, sending_time) -> 2;
field_index(list_strike_price, list_id) -> 3;
field_index(list_strike_price, tot_no_strikes) -> 4;
field_index(list_strike_price, last_fragment) -> 5;
field_index(registration_instructions, sending_time) -> 2;
field_index(registration_instructions, regist_id) -> 3;
field_index(registration_instructions, regist_trans_type) -> 4;
field_index(registration_instructions, regist_ref_id) -> 5;
field_index(registration_instructions, cl_ord_id) -> 6;
field_index(registration_instructions, account) -> 7;
field_index(registration_instructions, acct_id_source) -> 8;
field_index(registration_instructions, regist_acct_type) -> 9;
field_index(registration_instructions, tax_advantage_type) -> 10;
field_index(registration_instructions, ownership_type) -> 11;
field_index(registration_instructions_response, sending_time) -> 2;
field_index(registration_instructions_response, regist_id) -> 3;
field_index(registration_instructions_response, regist_trans_type) -> 4;
field_index(registration_instructions_response, regist_ref_id) -> 5;
field_index(registration_instructions_response, cl_ord_id) -> 6;
field_index(registration_instructions_response, account) -> 7;
field_index(registration_instructions_response, acct_id_source) -> 8;
field_index(registration_instructions_response, regist_status) -> 9;
field_index(registration_instructions_response, regist_rej_reason_code) -> 10;
field_index(registration_instructions_response, regist_rej_reason_text) -> 11;
field_index(order_mass_cancel_request, sending_time) -> 2;
field_index(order_mass_cancel_request, cl_ord_id) -> 3;
field_index(order_mass_cancel_request, secondary_cl_ord_id) -> 4;
field_index(order_mass_cancel_request, mass_cancel_request_type) -> 5;
field_index(order_mass_cancel_request, trading_session_id) -> 6;
field_index(order_mass_cancel_request, trading_session_sub_id) -> 7;
field_index(order_mass_cancel_request, side) -> 8;
field_index(order_mass_cancel_request, transact_time) -> 9;
field_index(order_mass_cancel_request, text) -> 10;
field_index(order_mass_cancel_request, encoded_text) -> 11;
field_index(order_mass_cancel_report, sending_time) -> 2;
field_index(order_mass_cancel_report, cl_ord_id) -> 3;
field_index(order_mass_cancel_report, secondary_cl_ord_id) -> 4;
field_index(order_mass_cancel_report, order_id) -> 5;
field_index(order_mass_cancel_report, secondary_order_id) -> 6;
field_index(order_mass_cancel_report, mass_cancel_request_type) -> 7;
field_index(order_mass_cancel_report, mass_cancel_response) -> 8;
field_index(order_mass_cancel_report, mass_cancel_reject_reason) -> 9;
field_index(order_mass_cancel_report, total_affected_orders) -> 10;
field_index(order_mass_cancel_report, trading_session_id) -> 11;
field_index(order_mass_cancel_report, trading_session_sub_id) -> 12;
field_index(order_mass_cancel_report, side) -> 13;
field_index(order_mass_cancel_report, transact_time) -> 14;
field_index(order_mass_cancel_report, text) -> 15;
field_index(order_mass_cancel_report, encoded_text) -> 16;
field_index(new_order_cross, sending_time) -> 2;
field_index(new_order_cross, cross_id) -> 3;
field_index(new_order_cross, cross_type) -> 4;
field_index(new_order_cross, cross_prioritization) -> 5;
field_index(new_order_cross, qty_type) -> 6;
field_index(new_order_cross, order_capacity) -> 7;
field_index(new_order_cross, order_restrictions) -> 8;
field_index(new_order_cross, cust_order_capacity) -> 9;
field_index(new_order_cross, forex_req) -> 10;
field_index(new_order_cross, settl_currency) -> 11;
field_index(new_order_cross, booking_type) -> 12;
field_index(new_order_cross, text) -> 13;
field_index(new_order_cross, encoded_text) -> 14;
field_index(new_order_cross, position_effect) -> 15;
field_index(new_order_cross, covered_or_uncovered) -> 16;
field_index(new_order_cross, cash_margin) -> 17;
field_index(new_order_cross, clearing_fee_indicator) -> 18;
field_index(new_order_cross, solicited_flag) -> 19;
field_index(new_order_cross, side_compliance_id) -> 20;
field_index(new_order_cross, settl_type) -> 21;
field_index(new_order_cross, settl_date) -> 22;
field_index(new_order_cross, handl_inst) -> 23;
field_index(new_order_cross, exec_inst) -> 24;
field_index(new_order_cross, min_qty) -> 25;
field_index(new_order_cross, max_floor) -> 26;
field_index(new_order_cross, ex_destination) -> 27;
field_index(new_order_cross, process_code) -> 28;
field_index(new_order_cross, prev_close_px) -> 29;
field_index(new_order_cross, locate_reqd) -> 30;
field_index(new_order_cross, transact_time) -> 31;
field_index(new_order_cross, ord_type) -> 32;
field_index(new_order_cross, price_type) -> 33;
field_index(new_order_cross, price) -> 34;
field_index(new_order_cross, stop_px) -> 35;
field_index(new_order_cross, currency) -> 36;
field_index(new_order_cross, compliance_id) -> 37;
field_index(new_order_cross, ioi_id) -> 38;
field_index(new_order_cross, quote_id) -> 39;
field_index(new_order_cross, time_in_force) -> 40;
field_index(new_order_cross, effective_time) -> 41;
field_index(new_order_cross, expire_date) -> 42;
field_index(new_order_cross, expire_time) -> 43;
field_index(new_order_cross, gt_booking_inst) -> 44;
field_index(new_order_cross, max_show) -> 45;
field_index(new_order_cross, target_strategy) -> 46;
field_index(new_order_cross, target_strategy_parameters) -> 47;
field_index(new_order_cross, participation_rate) -> 48;
field_index(new_order_cross, cancellation_rights) -> 49;
field_index(new_order_cross, money_laundering_status) -> 50;
field_index(new_order_cross, regist_id) -> 51;
field_index(new_order_cross, designation) -> 52;
field_index(cross_order_cancel_replace_request, sending_time) -> 2;
field_index(cross_order_cancel_replace_request, order_id) -> 3;
field_index(cross_order_cancel_replace_request, cross_id) -> 4;
field_index(cross_order_cancel_replace_request, orig_cross_id) -> 5;
field_index(cross_order_cancel_replace_request, cross_type) -> 6;
field_index(cross_order_cancel_replace_request, cross_prioritization) -> 7;
field_index(cross_order_cancel_replace_request, qty_type) -> 8;
field_index(cross_order_cancel_replace_request, order_capacity) -> 9;
field_index(cross_order_cancel_replace_request, order_restrictions) -> 10;
field_index(cross_order_cancel_replace_request, cust_order_capacity) -> 11;
field_index(cross_order_cancel_replace_request, forex_req) -> 12;
field_index(cross_order_cancel_replace_request, settl_currency) -> 13;
field_index(cross_order_cancel_replace_request, booking_type) -> 14;
field_index(cross_order_cancel_replace_request, text) -> 15;
field_index(cross_order_cancel_replace_request, encoded_text) -> 16;
field_index(cross_order_cancel_replace_request, position_effect) -> 17;
field_index(cross_order_cancel_replace_request, covered_or_uncovered) -> 18;
field_index(cross_order_cancel_replace_request, cash_margin) -> 19;
field_index(cross_order_cancel_replace_request, clearing_fee_indicator) -> 20;
field_index(cross_order_cancel_replace_request, solicited_flag) -> 21;
field_index(cross_order_cancel_replace_request, side_compliance_id) -> 22;
field_index(cross_order_cancel_replace_request, settl_type) -> 23;
field_index(cross_order_cancel_replace_request, settl_date) -> 24;
field_index(cross_order_cancel_replace_request, handl_inst) -> 25;
field_index(cross_order_cancel_replace_request, exec_inst) -> 26;
field_index(cross_order_cancel_replace_request, min_qty) -> 27;
field_index(cross_order_cancel_replace_request, max_floor) -> 28;
field_index(cross_order_cancel_replace_request, ex_destination) -> 29;
field_index(cross_order_cancel_replace_request, process_code) -> 30;
field_index(cross_order_cancel_replace_request, prev_close_px) -> 31;
field_index(cross_order_cancel_replace_request, locate_reqd) -> 32;
field_index(cross_order_cancel_replace_request, transact_time) -> 33;
field_index(cross_order_cancel_replace_request, ord_type) -> 34;
field_index(cross_order_cancel_replace_request, price_type) -> 35;
field_index(cross_order_cancel_replace_request, price) -> 36;
field_index(cross_order_cancel_replace_request, stop_px) -> 37;
field_index(cross_order_cancel_replace_request, currency) -> 38;
field_index(cross_order_cancel_replace_request, compliance_id) -> 39;
field_index(cross_order_cancel_replace_request, ioi_id) -> 40;
field_index(cross_order_cancel_replace_request, quote_id) -> 41;
field_index(cross_order_cancel_replace_request, time_in_force) -> 42;
field_index(cross_order_cancel_replace_request, effective_time) -> 43;
field_index(cross_order_cancel_replace_request, expire_date) -> 44;
field_index(cross_order_cancel_replace_request, expire_time) -> 45;
field_index(cross_order_cancel_replace_request, gt_booking_inst) -> 46;
field_index(cross_order_cancel_replace_request, max_show) -> 47;
field_index(cross_order_cancel_replace_request, target_strategy) -> 48;
field_index(cross_order_cancel_replace_request, target_strategy_parameters) -> 49;
field_index(cross_order_cancel_replace_request, participation_rate) -> 50;
field_index(cross_order_cancel_replace_request, cancellation_rights) -> 51;
field_index(cross_order_cancel_replace_request, money_laundering_status) -> 52;
field_index(cross_order_cancel_replace_request, regist_id) -> 53;
field_index(cross_order_cancel_replace_request, designation) -> 54;
field_index(cross_order_cancel_request, sending_time) -> 2;
field_index(cross_order_cancel_request, order_id) -> 3;
field_index(cross_order_cancel_request, cross_id) -> 4;
field_index(cross_order_cancel_request, orig_cross_id) -> 5;
field_index(cross_order_cancel_request, cross_type) -> 6;
field_index(cross_order_cancel_request, cross_prioritization) -> 7;
field_index(cross_order_cancel_request, transact_time) -> 8;
field_index(security_type_request, sending_time) -> 2;
field_index(security_type_request, security_req_id) -> 3;
field_index(security_type_request, text) -> 4;
field_index(security_type_request, encoded_text) -> 5;
field_index(security_type_request, trading_session_id) -> 6;
field_index(security_type_request, trading_session_sub_id) -> 7;
field_index(security_type_request, product) -> 8;
field_index(security_type_request, security_type) -> 9;
field_index(security_type_request, security_sub_type) -> 10;
field_index(security_types, sending_time) -> 2;
field_index(security_types, security_req_id) -> 3;
field_index(security_types, security_response_id) -> 4;
field_index(security_types, security_response_type) -> 5;
field_index(security_types, tot_no_security_types) -> 6;
field_index(security_types, last_fragment) -> 7;
field_index(security_types, text) -> 8;
field_index(security_types, encoded_text) -> 9;
field_index(security_types, trading_session_id) -> 10;
field_index(security_types, trading_session_sub_id) -> 11;
field_index(security_types, subscription_request_type) -> 12;
field_index(security_list_request, sending_time) -> 2;
field_index(security_list_request, security_req_id) -> 3;
field_index(security_list_request, security_list_request_type) -> 4;
field_index(security_list_request, currency) -> 5;
field_index(security_list_request, text) -> 6;
field_index(security_list_request, encoded_text) -> 7;
field_index(security_list_request, trading_session_id) -> 8;
field_index(security_list_request, trading_session_sub_id) -> 9;
field_index(security_list_request, subscription_request_type) -> 10;
field_index(security_list, sending_time) -> 2;
field_index(security_list, security_req_id) -> 3;
field_index(security_list, security_response_id) -> 4;
field_index(security_list, security_request_result) -> 5;
field_index(security_list, tot_no_related_sym) -> 6;
field_index(security_list, last_fragment) -> 7;
field_index(security_list, currency) -> 8;
field_index(security_list, round_lot) -> 9;
field_index(security_list, min_trade_vol) -> 10;
field_index(security_list, trading_session_id) -> 11;
field_index(security_list, trading_session_sub_id) -> 12;
field_index(security_list, expiration_cycle) -> 13;
field_index(security_list, text) -> 14;
field_index(security_list, encoded_text) -> 15;
field_index(derivative_security_list_request, sending_time) -> 2;
field_index(derivative_security_list_request, security_req_id) -> 3;
field_index(derivative_security_list_request, security_list_request_type) -> 4;
field_index(derivative_security_list_request, security_sub_type) -> 5;
field_index(derivative_security_list_request, currency) -> 6;
field_index(derivative_security_list_request, text) -> 7;
field_index(derivative_security_list_request, encoded_text) -> 8;
field_index(derivative_security_list_request, trading_session_id) -> 9;
field_index(derivative_security_list_request, trading_session_sub_id) -> 10;
field_index(derivative_security_list_request, subscription_request_type) -> 11;
field_index(derivative_security_list, sending_time) -> 2;
field_index(derivative_security_list, security_req_id) -> 3;
field_index(derivative_security_list, security_response_id) -> 4;
field_index(derivative_security_list, security_request_result) -> 5;
field_index(derivative_security_list, tot_no_related_sym) -> 6;
field_index(derivative_security_list, last_fragment) -> 7;
field_index(derivative_security_list, trading_session_id) -> 8;
field_index(derivative_security_list, trading_session_sub_id) -> 9;
field_index(derivative_security_list, text) -> 10;
field_index(derivative_security_list, encoded_text) -> 11;
field_index(new_order_multileg, sending_time) -> 2;
field_index(new_order_multileg, cl_ord_id) -> 3;
field_index(new_order_multileg, secondary_cl_ord_id) -> 4;
field_index(new_order_multileg, cl_ord_link_id) -> 5;
field_index(new_order_multileg, trade_origination_date) -> 6;
field_index(new_order_multileg, trade_date) -> 7;
field_index(new_order_multileg, account) -> 8;
field_index(new_order_multileg, acct_id_source) -> 9;
field_index(new_order_multileg, account_type) -> 10;
field_index(new_order_multileg, day_booking_inst) -> 11;
field_index(new_order_multileg, booking_unit) -> 12;
field_index(new_order_multileg, prealloc_method) -> 13;
field_index(new_order_multileg, alloc_id) -> 14;
field_index(new_order_multileg, settl_type) -> 15;
field_index(new_order_multileg, settl_date) -> 16;
field_index(new_order_multileg, cash_margin) -> 17;
field_index(new_order_multileg, clearing_fee_indicator) -> 18;
field_index(new_order_multileg, handl_inst) -> 19;
field_index(new_order_multileg, exec_inst) -> 20;
field_index(new_order_multileg, min_qty) -> 21;
field_index(new_order_multileg, max_floor) -> 22;
field_index(new_order_multileg, ex_destination) -> 23;
field_index(new_order_multileg, process_code) -> 24;
field_index(new_order_multileg, side) -> 25;
field_index(new_order_multileg, prev_close_px) -> 26;
field_index(new_order_multileg, leg_position_effect) -> 27;
field_index(new_order_multileg, leg_covered_or_uncovered) -> 28;
field_index(new_order_multileg, leg_ref_id) -> 29;
field_index(new_order_multileg, leg_price) -> 30;
field_index(new_order_multileg, leg_settl_type) -> 31;
field_index(new_order_multileg, leg_settl_date) -> 32;
field_index(new_order_multileg, locate_reqd) -> 33;
field_index(new_order_multileg, transact_time) -> 34;
field_index(new_order_multileg, qty_type) -> 35;
field_index(new_order_multileg, ord_type) -> 36;
field_index(new_order_multileg, price_type) -> 37;
field_index(new_order_multileg, price) -> 38;
field_index(new_order_multileg, stop_px) -> 39;
field_index(new_order_multileg, currency) -> 40;
field_index(new_order_multileg, compliance_id) -> 41;
field_index(new_order_multileg, solicited_flag) -> 42;
field_index(new_order_multileg, ioi_id) -> 43;
field_index(new_order_multileg, quote_id) -> 44;
field_index(new_order_multileg, time_in_force) -> 45;
field_index(new_order_multileg, effective_time) -> 46;
field_index(new_order_multileg, expire_date) -> 47;
field_index(new_order_multileg, expire_time) -> 48;
field_index(new_order_multileg, gt_booking_inst) -> 49;
field_index(new_order_multileg, order_capacity) -> 50;
field_index(new_order_multileg, order_restrictions) -> 51;
field_index(new_order_multileg, cust_order_capacity) -> 52;
field_index(new_order_multileg, forex_req) -> 53;
field_index(new_order_multileg, settl_currency) -> 54;
field_index(new_order_multileg, booking_type) -> 55;
field_index(new_order_multileg, text) -> 56;
field_index(new_order_multileg, encoded_text) -> 57;
field_index(new_order_multileg, position_effect) -> 58;
field_index(new_order_multileg, covered_or_uncovered) -> 59;
field_index(new_order_multileg, max_show) -> 60;
field_index(new_order_multileg, target_strategy) -> 61;
field_index(new_order_multileg, target_strategy_parameters) -> 62;
field_index(new_order_multileg, participation_rate) -> 63;
field_index(new_order_multileg, cancellation_rights) -> 64;
field_index(new_order_multileg, money_laundering_status) -> 65;
field_index(new_order_multileg, regist_id) -> 66;
field_index(new_order_multileg, designation) -> 67;
field_index(new_order_multileg, multi_leg_rpt_type_req) -> 68;
field_index(multileg_order_cancel_replace, sending_time) -> 2;
field_index(multileg_order_cancel_replace, order_id) -> 3;
field_index(multileg_order_cancel_replace, orig_cl_ord_id) -> 4;
field_index(multileg_order_cancel_replace, cl_ord_id) -> 5;
field_index(multileg_order_cancel_replace, secondary_cl_ord_id) -> 6;
field_index(multileg_order_cancel_replace, cl_ord_link_id) -> 7;
field_index(multileg_order_cancel_replace, orig_ord_mod_time) -> 8;
field_index(multileg_order_cancel_replace, trade_origination_date) -> 9;
field_index(multileg_order_cancel_replace, trade_date) -> 10;
field_index(multileg_order_cancel_replace, account) -> 11;
field_index(multileg_order_cancel_replace, acct_id_source) -> 12;
field_index(multileg_order_cancel_replace, account_type) -> 13;
field_index(multileg_order_cancel_replace, day_booking_inst) -> 14;
field_index(multileg_order_cancel_replace, booking_unit) -> 15;
field_index(multileg_order_cancel_replace, prealloc_method) -> 16;
field_index(multileg_order_cancel_replace, alloc_id) -> 17;
field_index(multileg_order_cancel_replace, settl_type) -> 18;
field_index(multileg_order_cancel_replace, settl_date) -> 19;
field_index(multileg_order_cancel_replace, cash_margin) -> 20;
field_index(multileg_order_cancel_replace, clearing_fee_indicator) -> 21;
field_index(multileg_order_cancel_replace, handl_inst) -> 22;
field_index(multileg_order_cancel_replace, exec_inst) -> 23;
field_index(multileg_order_cancel_replace, min_qty) -> 24;
field_index(multileg_order_cancel_replace, max_floor) -> 25;
field_index(multileg_order_cancel_replace, ex_destination) -> 26;
field_index(multileg_order_cancel_replace, process_code) -> 27;
field_index(multileg_order_cancel_replace, side) -> 28;
field_index(multileg_order_cancel_replace, prev_close_px) -> 29;
field_index(multileg_order_cancel_replace, leg_position_effect) -> 30;
field_index(multileg_order_cancel_replace, leg_covered_or_uncovered) -> 31;
field_index(multileg_order_cancel_replace, leg_ref_id) -> 32;
field_index(multileg_order_cancel_replace, leg_price) -> 33;
field_index(multileg_order_cancel_replace, leg_settl_type) -> 34;
field_index(multileg_order_cancel_replace, leg_settl_date) -> 35;
field_index(multileg_order_cancel_replace, locate_reqd) -> 36;
field_index(multileg_order_cancel_replace, transact_time) -> 37;
field_index(multileg_order_cancel_replace, qty_type) -> 38;
field_index(multileg_order_cancel_replace, ord_type) -> 39;
field_index(multileg_order_cancel_replace, price_type) -> 40;
field_index(multileg_order_cancel_replace, price) -> 41;
field_index(multileg_order_cancel_replace, stop_px) -> 42;
field_index(multileg_order_cancel_replace, currency) -> 43;
field_index(multileg_order_cancel_replace, compliance_id) -> 44;
field_index(multileg_order_cancel_replace, solicited_flag) -> 45;
field_index(multileg_order_cancel_replace, ioi_id) -> 46;
field_index(multileg_order_cancel_replace, quote_id) -> 47;
field_index(multileg_order_cancel_replace, time_in_force) -> 48;
field_index(multileg_order_cancel_replace, effective_time) -> 49;
field_index(multileg_order_cancel_replace, expire_date) -> 50;
field_index(multileg_order_cancel_replace, expire_time) -> 51;
field_index(multileg_order_cancel_replace, gt_booking_inst) -> 52;
field_index(multileg_order_cancel_replace, order_capacity) -> 53;
field_index(multileg_order_cancel_replace, order_restrictions) -> 54;
field_index(multileg_order_cancel_replace, cust_order_capacity) -> 55;
field_index(multileg_order_cancel_replace, forex_req) -> 56;
field_index(multileg_order_cancel_replace, settl_currency) -> 57;
field_index(multileg_order_cancel_replace, booking_type) -> 58;
field_index(multileg_order_cancel_replace, text) -> 59;
field_index(multileg_order_cancel_replace, encoded_text) -> 60;
field_index(multileg_order_cancel_replace, position_effect) -> 61;
field_index(multileg_order_cancel_replace, covered_or_uncovered) -> 62;
field_index(multileg_order_cancel_replace, max_show) -> 63;
field_index(multileg_order_cancel_replace, target_strategy) -> 64;
field_index(multileg_order_cancel_replace, target_strategy_parameters) -> 65;
field_index(multileg_order_cancel_replace, participation_rate) -> 66;
field_index(multileg_order_cancel_replace, cancellation_rights) -> 67;
field_index(multileg_order_cancel_replace, money_laundering_status) -> 68;
field_index(multileg_order_cancel_replace, regist_id) -> 69;
field_index(multileg_order_cancel_replace, designation) -> 70;
field_index(multileg_order_cancel_replace, multi_leg_rpt_type_req) -> 71;
field_index(trade_capture_report_request, sending_time) -> 2;
field_index(trade_capture_report_request, trade_request_id) -> 3;
field_index(trade_capture_report_request, trade_request_type) -> 4;
field_index(trade_capture_report_request, subscription_request_type) -> 5;
field_index(trade_capture_report_request, trade_report_id) -> 6;
field_index(trade_capture_report_request, secondary_trade_report_id) -> 7;
field_index(trade_capture_report_request, exec_id) -> 8;
field_index(trade_capture_report_request, exec_type) -> 9;
field_index(trade_capture_report_request, order_id) -> 10;
field_index(trade_capture_report_request, cl_ord_id) -> 11;
field_index(trade_capture_report_request, match_status) -> 12;
field_index(trade_capture_report_request, trd_type) -> 13;
field_index(trade_capture_report_request, trd_sub_type) -> 14;
field_index(trade_capture_report_request, transfer_reason) -> 15;
field_index(trade_capture_report_request, secondary_trd_type) -> 16;
field_index(trade_capture_report_request, trade_link_id) -> 17;
field_index(trade_capture_report_request, trd_match_id) -> 18;
field_index(trade_capture_report_request, clearing_business_date) -> 19;
field_index(trade_capture_report_request, trading_session_id) -> 20;
field_index(trade_capture_report_request, trading_session_sub_id) -> 21;
field_index(trade_capture_report_request, time_bracket) -> 22;
field_index(trade_capture_report_request, side) -> 23;
field_index(trade_capture_report_request, multi_leg_reporting_type) -> 24;
field_index(trade_capture_report_request, trade_input_source) -> 25;
field_index(trade_capture_report_request, trade_input_device) -> 26;
field_index(trade_capture_report_request, response_transport_type) -> 27;
field_index(trade_capture_report_request, response_destination) -> 28;
field_index(trade_capture_report_request, text) -> 29;
field_index(trade_capture_report_request, encoded_text) -> 30;
field_index(trade_capture_report, sending_time) -> 2;
field_index(trade_capture_report, trade_report_id) -> 3;
field_index(trade_capture_report, trade_report_trans_type) -> 4;
field_index(trade_capture_report, trade_report_type) -> 5;
field_index(trade_capture_report, trade_request_id) -> 6;
field_index(trade_capture_report, trd_type) -> 7;
field_index(trade_capture_report, trd_sub_type) -> 8;
field_index(trade_capture_report, secondary_trd_type) -> 9;
field_index(trade_capture_report, transfer_reason) -> 10;
field_index(trade_capture_report, exec_type) -> 11;
field_index(trade_capture_report, tot_num_trade_reports) -> 12;
field_index(trade_capture_report, last_rpt_requested) -> 13;
field_index(trade_capture_report, unsolicited_indicator) -> 14;
field_index(trade_capture_report, subscription_request_type) -> 15;
field_index(trade_capture_report, trade_report_ref_id) -> 16;
field_index(trade_capture_report, secondary_trade_report_ref_id) -> 17;
field_index(trade_capture_report, secondary_trade_report_id) -> 18;
field_index(trade_capture_report, trade_link_id) -> 19;
field_index(trade_capture_report, trd_match_id) -> 20;
field_index(trade_capture_report, exec_id) -> 21;
field_index(trade_capture_report, ord_status) -> 22;
field_index(trade_capture_report, secondary_exec_id) -> 23;
field_index(trade_capture_report, exec_restatement_reason) -> 24;
field_index(trade_capture_report, previously_reported) -> 25;
field_index(trade_capture_report, price_type) -> 26;
field_index(trade_capture_report, qty_type) -> 27;
field_index(trade_capture_report, underlying_trading_session_id) -> 28;
field_index(trade_capture_report, underlying_trading_session_sub_id) -> 29;
field_index(trade_capture_report, last_qty) -> 30;
field_index(trade_capture_report, last_px) -> 31;
field_index(trade_capture_report, last_par_px) -> 32;
field_index(trade_capture_report, last_spot_rate) -> 33;
field_index(trade_capture_report, last_forward_points) -> 34;
field_index(trade_capture_report, last_mkt) -> 35;
field_index(trade_capture_report, trade_date) -> 36;
field_index(trade_capture_report, clearing_business_date) -> 37;
field_index(trade_capture_report, avg_px) -> 38;
field_index(trade_capture_report, avg_px_indicator) -> 39;
field_index(trade_capture_report, multi_leg_reporting_type) -> 40;
field_index(trade_capture_report, trade_leg_ref_id) -> 41;
field_index(trade_capture_report, transact_time) -> 42;
field_index(trade_capture_report, settl_type) -> 43;
field_index(trade_capture_report, settl_date) -> 44;
field_index(trade_capture_report, match_status) -> 45;
field_index(trade_capture_report, match_type) -> 46;
field_index(trade_capture_report, clearing_fee_indicator) -> 47;
field_index(trade_capture_report, trade_input_source) -> 48;
field_index(trade_capture_report, trade_input_device) -> 49;
field_index(trade_capture_report, order_input_device) -> 50;
field_index(trade_capture_report, currency) -> 51;
field_index(trade_capture_report, compliance_id) -> 52;
field_index(trade_capture_report, solicited_flag) -> 53;
field_index(trade_capture_report, order_capacity) -> 54;
field_index(trade_capture_report, order_restrictions) -> 55;
field_index(trade_capture_report, cust_order_capacity) -> 56;
field_index(trade_capture_report, ord_type) -> 57;
field_index(trade_capture_report, exec_inst) -> 58;
field_index(trade_capture_report, trans_bkd_time) -> 59;
field_index(trade_capture_report, trading_session_id) -> 60;
field_index(trade_capture_report, trading_session_sub_id) -> 61;
field_index(trade_capture_report, time_bracket) -> 62;
field_index(trade_capture_report, gross_trade_amt) -> 63;
field_index(trade_capture_report, num_days_interest) -> 64;
field_index(trade_capture_report, ex_date) -> 65;
field_index(trade_capture_report, accrued_interest_rate) -> 66;
field_index(trade_capture_report, accrued_interest_amt) -> 67;
field_index(trade_capture_report, interest_at_maturity) -> 68;
field_index(trade_capture_report, end_accrued_interest_amt) -> 69;
field_index(trade_capture_report, start_cash) -> 70;
field_index(trade_capture_report, end_cash) -> 71;
field_index(trade_capture_report, concession) -> 72;
field_index(trade_capture_report, total_takedown) -> 73;
field_index(trade_capture_report, net_money) -> 74;
field_index(trade_capture_report, settl_curr_amt) -> 75;
field_index(trade_capture_report, settl_currency) -> 76;
field_index(trade_capture_report, settl_curr_fx_rate) -> 77;
field_index(trade_capture_report, settl_curr_fx_rate_calc) -> 78;
field_index(trade_capture_report, position_effect) -> 79;
field_index(trade_capture_report, text) -> 80;
field_index(trade_capture_report, encoded_text) -> 81;
field_index(trade_capture_report, side_multi_leg_reporting_type) -> 82;
field_index(trade_capture_report, exchange_rule) -> 83;
field_index(trade_capture_report, trade_alloc_indicator) -> 84;
field_index(trade_capture_report, prealloc_method) -> 85;
field_index(trade_capture_report, alloc_id) -> 86;
field_index(trade_capture_report, copy_msg_indicator) -> 87;
field_index(trade_capture_report, publish_trd_indicator) -> 88;
field_index(trade_capture_report, short_sale_reason) -> 89;
...
field_index(rfq_request, sending_time) -> 2;
field_index(rfq_request, rfq_req_id) -> 3;
field_index(rfq_request, prev_close_px) -> 4;
field_index(rfq_request, quote_request_type) -> 5;
field_index(rfq_request, quote_type) -> 6;
field_index(rfq_request, trading_session_id) -> 7;
field_index(rfq_request, trading_session_sub_id) -> 8;
field_index(rfq_request, subscription_request_type) -> 9;
...
field_index(quote_response, sending_time) -> 2;
field_index(quote_response, quote_resp_id) -> 3;
field_index(quote_response, quote_id) -> 4;
field_index(quote_response, quote_resp_type) -> 5;
field_index(quote_response, cl_ord_id) -> 6;
field_index(quote_response, order_capacity) -> 7;
field_index(quote_response, ioi_id) -> 8;
field_index(quote_response, quote_type) -> 9;
field_index(quote_response, trading_session_id) -> 10;
field_index(quote_response, trading_session_sub_id) -> 11;
field_index(quote_response, side) -> 12;
field_index(quote_response, settl_type) -> 13;
field_index(quote_response, settl_date) -> 14;
field_index(quote_response, settl_date2) -> 15;
field_index(quote_response, order_qty2) -> 16;
field_index(quote_response, currency) -> 17;
field_index(quote_response, account) -> 18;
field_index(quote_response, acct_id_source) -> 19;
field_index(quote_response, account_type) -> 20;
field_index(quote_response, bid_px) -> 21;
field_index(quote_response, offer_px) -> 22;
field_index(quote_response, mkt_bid_px) -> 23;
field_index(quote_response, mkt_offer_px) -> 24;
field_index(quote_response, min_bid_size) -> 25;
field_index(quote_response, bid_size) -> 26;
field_index(quote_response, min_offer_size) -> 27;
field_index(quote_response, offer_size) -> 28;
field_index(quote_response, valid_until_time) -> 29;
field_index(quote_response, bid_spot_rate) -> 30;
field_index(quote_response, offer_spot_rate) -> 31;
field_index(quote_response, bid_forward_points) -> 32;
field_index(quote_response, offer_forward_points) -> 33;
field_index(quote_response, mid_px) -> 34;
field_index(quote_response, bid_yield) -> 35;
field_index(quote_response, mid_yield) -> 36;
field_index(quote_response, offer_yield) -> 37;
field_index(quote_response, transact_time) -> 38;
field_index(quote_response, ord_type) -> 39;
field_index(quote_response, bid_forward_points2) -> 40;
field_index(quote_response, offer_forward_points2) -> 41;
field_index(quote_response, settl_curr_bid_fx_rate) -> 42;
field_index(quote_response, settl_curr_offer_fx_rate) -> 43;
field_index(quote_response, settl_curr_fx_rate_calc) -> 44;
field_index(quote_response, commission) -> 45;
field_index(quote_response, comm_type) -> 46;
field_index(quote_response, cust_order_capacity) -> 47;
field_index(quote_response, ex_destination) -> 48;
field_index(quote_response, text) -> 49;
field_index(quote_response, encoded_text) -> 50;
field_index(quote_response, price) -> 51;
field_index(quote_response, price_type) -> 52;
field_index(confirmation, sending_time) -> 2;
field_index(confirmation, confirm_id) -> 3;
field_index(confirmation, confirm_ref_id) -> 4;
field_index(confirmation, confirm_req_id) -> 5;
field_index(confirmation, confirm_trans_type) -> 6;
field_index(confirmation, confirm_type) -> 7;
field_index(confirmation, copy_msg_indicator) -> 8;
field_index(confirmation, legal_confirm) -> 9;
field_index(confirmation, confirm_status) -> 10;
field_index(confirmation, alloc_id) -> 11;
field_index(confirmation, secondary_alloc_id) -> 12;
field_index(confirmation, individual_alloc_id) -> 13;
field_index(confirmation, transact_time) -> 14;
field_index(confirmation, trade_date) -> 15;
field_index(confirmation, alloc_qty) -> 16;
field_index(confirmation, qty_type) -> 17;
field_index(confirmation, side) -> 18;
field_index(confirmation, currency) -> 19;
field_index(confirmation, last_mkt) -> 20;
field_index(confirmation, alloc_account) -> 21;
field_index(confirmation, alloc_acct_id_source) -> 22;
field_index(confirmation, alloc_account_type) -> 23;
field_index(confirmation, avg_px) -> 24;
field_index(confirmation, avg_px_precision) -> 25;
field_index(confirmation, price_type) -> 26;
field_index(confirmation, avg_par_px) -> 27;
field_index(confirmation, reported_px) -> 28;
field_index(confirmation, text) -> 29;
field_index(confirmation, encoded_text) -> 30;
field_index(confirmation, process_code) -> 31;
field_index(confirmation, gross_trade_amt) -> 32;
field_index(confirmation, num_days_interest) -> 33;
field_index(confirmation, ex_date) -> 34;
field_index(confirmation, accrued_interest_rate) -> 35;
field_index(confirmation, accrued_interest_amt) -> 36;
field_index(confirmation, interest_at_maturity) -> 37;
field_index(confirmation, end_accrued_interest_amt) -> 38;
field_index(confirmation, start_cash) -> 39;
field_index(confirmation, end_cash) -> 40;
field_index(confirmation, concession) -> 41;
field_index(confirmation, total_takedown) -> 42;
field_index(confirmation, net_money) -> 43;
field_index(confirmation, maturity_net_money) -> 44;
field_index(confirmation, settl_curr_amt) -> 45;
field_index(confirmation, settl_currency) -> 46;
field_index(confirmation, settl_curr_fx_rate) -> 47;
field_index(confirmation, settl_curr_fx_rate_calc) -> 48;
field_index(confirmation, settl_type) -> 49;
field_index(confirmation, settl_date) -> 50;
field_index(confirmation, shared_commission) -> 51;
...
field_index(request_for_positions, match_status) -> 5;
field_index(request_for_positions, subscription_request_type) -> 6;
field_index(request_for_positions, account) -> 7;
field_index(request_for_positions, acct_id_source) -> 8;
field_index(request_for_positions, account_type) -> 9;
field_index(request_for_positions, currency) -> 10;
field_index(request_for_positions, clearing_business_date) -> 11;
field_index(request_for_positions, settl_sess_id) -> 12;
field_index(request_for_positions, settl_sess_sub_id) -> 13;
field_index(request_for_positions, transact_time) -> 14;
field_index(request_for_positions, response_transport_type) -> 15;
field_index(request_for_positions, response_destination) -> 16;
field_index(request_for_positions, text) -> 17;
field_index(request_for_positions, encoded_text) -> 18;
field_index(request_for_positions_ack, sending_time) -> 2;
field_index(request_for_positions_ack, pos_maint_rpt_id) -> 3;
field_index(request_for_positions_ack, pos_req_id) -> 4;
field_index(request_for_positions_ack, total_num_pos_reports) -> 5;
field_index(request_for_positions_ack, unsolicited_indicator) -> 6;
field_index(request_for_positions_ack, pos_req_result) -> 7;
field_index(request_for_positions_ack, pos_req_status) -> 8;
field_index(request_for_positions_ack, account) -> 9;
field_index(request_for_positions_ack, acct_id_source) -> 10;
field_index(request_for_positions_ack, account_type) -> 11;
field_index(request_for_positions_ack, currency) -> 12;
field_index(request_for_positions_ack, response_transport_type) -> 13;
field_index(request_for_positions_ack, response_destination) -> 14;
field_index(request_for_positions_ack, text) -> 15;
field_index(request_for_positions_ack, encoded_text) -> 16;
field_index(position_report, sending_time) -> 2;
field_index(position_report, pos_maint_rpt_id) -> 3;
field_index(position_report, pos_req_id) -> 4;
field_index(position_report, pos_req_type) -> 5;
field_index(position_report, subscription_request_type) -> 6;
field_index(position_report, total_num_pos_reports) -> 7;
field_index(position_report, unsolicited_indicator) -> 8;
field_index(position_report, pos_req_result) -> 9;
field_index(position_report, clearing_business_date) -> 10;
field_index(position_report, settl_sess_id) -> 11;
field_index(position_report, settl_sess_sub_id) -> 12;
field_index(position_report, account) -> 13;
field_index(position_report, acct_id_source) -> 14;
field_index(position_report, account_type) -> 15;
field_index(position_report, currency) -> 16;
field_index(position_report, settl_price) -> 17;
field_index(position_report, settl_price_type) -> 18;
field_index(position_report, prior_settl_price) -> 19;
field_index(position_report, regist_status) -> 20;
field_index(position_report, delivery_date) -> 21;
field_index(position_report, text) -> 22;
field_index(position_report, encoded_text) -> 23;
field_index(trade_capture_report_request_ack, sending_time) -> 2;
field_index(trade_capture_report_request_ack, trade_request_id) -> 3;
field_index(trade_capture_report_request_ack, trade_request_type) -> 4;
field_index(trade_capture_report_request_ack, subscription_request_type) -> 5;
field_index(trade_capture_report_request_ack, tot_num_trade_reports) -> 6;
field_index(trade_capture_report_request_ack, trade_request_result) -> 7;
field_index(trade_capture_report_request_ack, trade_request_status) -> 8;
field_index(trade_capture_report_request_ack, multi_leg_reporting_type) -> 9;
field_index(trade_capture_report_request_ack, response_transport_type) -> 10;
field_index(trade_capture_report_request_ack, response_destination) -> 11;
field_index(trade_capture_report_request_ack, text) -> 12;
field_index(trade_capture_report_request_ack, encoded_text) -> 13;
field_index(trade_capture_report_ack, sending_time) -> 2;
field_index(trade_capture_report_ack, trade_report_id) -> 3;
field_index(trade_capture_report_ack, trade_report_trans_type) -> 4;
field_index(trade_capture_report_ack, trade_report_type) -> 5;
field_index(trade_capture_report_ack, trd_type) -> 6;
field_index(trade_capture_report_ack, trd_sub_type) -> 7;
field_index(trade_capture_report_ack, secondary_trd_type) -> 8;
field_index(trade_capture_report_ack, transfer_reason) -> 9;
field_index(trade_capture_report_ack, exec_type) -> 10;
field_index(trade_capture_report_ack, trade_report_ref_id) -> 11;
field_index(trade_capture_report_ack, secondary_trade_report_ref_id) -> 12;
field_index(trade_capture_report_ack, trd_rpt_status) -> 13;
field_index(trade_capture_report_ack, trade_report_reject_reason) -> 14;
field_index(trade_capture_report_ack, secondary_trade_report_id) -> 15;
field_index(trade_capture_report_ack, subscription_request_type) -> 16;
field_index(trade_capture_report_ack, trade_link_id) -> 17;
field_index(trade_capture_report_ack, trd_match_id) -> 18;
field_index(trade_capture_report_ack, exec_id) -> 19;
field_index(trade_capture_report_ack, secondary_exec_id) -> 20;
field_index(trade_capture_report_ack, transact_time) -> 21;
field_index(trade_capture_report_ack, response_transport_type) -> 22;
field_index(trade_capture_report_ack, response_destination) -> 23;
field_index(trade_capture_report_ack, text) -> 24;
field_index(trade_capture_report_ack, encoded_text) -> 25;
field_index(trade_capture_report_ack, clearing_fee_indicator) -> 26;
field_index(trade_capture_report_ack, order_capacity) -> 27;
field_index(trade_capture_report_ack, order_restrictions) -> 28;
field_index(trade_capture_report_ack, cust_order_capacity) -> 29;
field_index(trade_capture_report_ack, account) -> 30;
field_index(trade_capture_report_ack, acct_id_source) -> 31;
field_index(trade_capture_report_ack, account_type) -> 32;
field_index(trade_capture_report_ack, position_effect) -> 33;
field_index(trade_capture_report_ack, prealloc_method) -> 34;
field_index(allocation_report, sending_time) -> 2;
field_index(allocation_report, alloc_report_id) -> 3;
field_index(allocation_report, alloc_id) -> 4;
field_index(allocation_report, alloc_trans_type) -> 5;
field_index(allocation_report, alloc_report_ref_id) -> 6;
field_index(allocation_report, alloc_canc_replace_reason) -> 7;
field_index(allocation_report, secondary_alloc_id) -> 8;
field_index(allocation_report, alloc_report_type) -> 9;
field_index(allocation_report, alloc_status) -> 10;
field_index(allocation_report, alloc_rej_code) -> 11;
field_index(allocation_report, ref_alloc_id) -> 12;
field_index(allocation_report, alloc_intermed_req_type) -> 13;
field_index(allocation_report, alloc_link_id) -> 14;
field_index(allocation_report, alloc_link_type) -> 15;
field_index(allocation_report, booking_ref_id) -> 16;
field_index(allocation_report, alloc_no_orders_type) -> 17;
field_index(allocation_report, previously_reported) -> 18;
field_index(allocation_report, reversal_indicator) -> 19;
field_index(allocation_report, match_type) -> 20;
field_index(allocation_report, side) -> 21;
field_index(allocation_report, quantity) -> 22;
field_index(allocation_report, qty_type) -> 23;
field_index(allocation_report, last_mkt) -> 24;
field_index(allocation_report, trade_origination_date) -> 25;
field_index(allocation_report, trading_session_id) -> 26;
field_index(allocation_report, trading_session_sub_id) -> 27;
field_index(allocation_report, price_type) -> 28;
field_index(allocation_report, avg_px) -> 29;
field_index(allocation_report, avg_par_px) -> 30;
field_index(allocation_report, currency) -> 31;
field_index(allocation_report, avg_px_precision) -> 32;
field_index(allocation_report, trade_date) -> 33;
field_index(allocation_report, transact_time) -> 34;
field_index(allocation_report, settl_type) -> 35;
field_index(allocation_report, settl_date) -> 36;
field_index(allocation_report, booking_type) -> 37;
field_index(allocation_report, gross_trade_amt) -> 38;
field_index(allocation_report, concession) -> 39;
field_index(allocation_report, total_takedown) -> 40;
field_index(allocation_report, net_money) -> 41;
field_index(allocation_report, position_effect) -> 42;
field_index(allocation_report, auto_accept_indicator) -> 43;
field_index(allocation_report, text) -> 44;
field_index(allocation_report, encoded_text) -> 45;
field_index(allocation_report, num_days_interest) -> 46;
field_index(allocation_report, accrued_interest_rate) -> 47;
field_index(allocation_report, accrued_interest_amt) -> 48;
field_index(allocation_report, total_accrued_interest_amt) -> 49;
field_index(allocation_report, interest_at_maturity) -> 50;
field_index(allocation_report, end_accrued_interest_amt) -> 51;
field_index(allocation_report, start_cash) -> 52;
field_index(allocation_report, end_cash) -> 53;
field_index(allocation_report, legal_confirm) -> 54;
field_index(allocation_report, tot_no_allocs) -> 55;
field_index(allocation_report, last_fragment) -> 56;
field_index(allocation_report, clearing_fee_indicator) -> 57;
field_index(allocation_report, alloc_settl_inst_type) -> 58;
field_index(allocation_report_ack, sending_time) -> 2;
field_index(allocation_report_ack, alloc_report_id) -> 3;
field_index(allocation_report_ack, alloc_id) -> 4;
field_index(allocation_report_ack, secondary_alloc_id) -> 5;
field_index(allocation_report_ack, trade_date) -> 6;
field_index(allocation_report_ack, transact_time) -> 7;
field_index(allocation_report_ack, alloc_status) -> 8;
field_index(allocation_report_ack, alloc_rej_code) -> 9;
field_index(allocation_report_ack, alloc_report_type) -> 10;
field_index(allocation_report_ack, alloc_intermed_req_type) -> 11;
field_index(allocation_report_ack, match_status) -> 12;
field_index(allocation_report_ack, product) -> 13;
field_index(allocation_report_ack, security_type) -> 14;
field_index(allocation_report_ack, text) -> 15;
field_index(allocation_report_ack, encoded_text) -> 16;
field_index(confirmation_ack, sending_time) -> 2;
field_index(confirmation_ack, confirm_id) -> 3;
field_index(confirmation_ack, trade_date) -> 4;
field_index(confirmation_ack, transact_time) -> 5;
field_index(confirmation_ack, affirm_status) -> 6;
field_index(confirmation_ack, confirm_rej_reason) -> 7;
field_index(confirmation_ack, match_status) -> 8;
field_index(confirmation_ack, text) -> 9;
field_index(confirmation_ack, encoded_text) -> 10;
field_index(settlement_instruction_request, sending_time) -> 2;
field_index(settlement_instruction_request, settl_inst_req_id) -> 3;
field_index(settlement_instruction_request, transact_time) -> 4;
field_index(settlement_instruction_request, alloc_account) -> 5;
field_index(settlement_instruction_request, alloc_acct_id_source) -> 6;
field_index(settlement_instruction_request, side) -> 7;
field_index(settlement_instruction_request, product) -> 8;
field_index(settlement_instruction_request, security_type) -> 9;
field_index(settlement_instruction_request, cfi_code) -> 10;
field_index(settlement_instruction_request, effective_time) -> 11;
field_index(settlement_instruction_request, expire_time) -> 12;
field_index(settlement_instruction_request, last_update_time) -> 13;
field_index(settlement_instruction_request, stand_inst_db_type) -> 14;
field_index(settlement_instruction_request, stand_inst_db_name) -> 15;
field_index(settlement_instruction_request, stand_inst_db_id) -> 16;
field_index(assignment_report, sending_time) -> 2;
field_index(assignment_report, asgn_rpt_id) -> 3;
field_index(assignment_report, tot_num_assignment_reports) -> 4;
field_index(assignment_report, last_rpt_requested) -> 5;
field_index(assignment_report, account) -> 6;
field_index(assignment_report, account_type) -> 7;
field_index(assignment_report, currency) -> 8;
field_index(assignment_report, threshold_amount) -> 9;
field_index(assignment_report, settl_price) -> 10;
field_index(assignment_report, settl_price_type) -> 11;
field_index(assignment_report, underlying_settl_price) -> 12;
field_index(assignment_report, expire_date) -> 13;
field_index(assignment_report, assignment_method) -> 14;
field_index(assignment_report, assignment_unit) -> 15;
field_index(assignment_report, open_interest) -> 16;
field_index(assignment_report, exercise_method) -> 17;
field_index(assignment_report, settl_sess_id) -> 18;
field_index(assignment_report, settl_sess_sub_id) -> 19;
field_index(assignment_report, clearing_business_date) -> 20;
field_index(assignment_report, text) -> 21;
field_index(assignment_report, encoded_text) -> 22;
field_index(collateral_request, sending_time) -> 2;
field_index(collateral_request, coll_req_id) -> 3;
field_index(collateral_request, coll_asgn_reason) -> 4;
field_index(collateral_request, transact_time) -> 5;
field_index(collateral_request, expire_time) -> 6;
field_index(collateral_request, account) -> 7;
field_index(collateral_request, account_type) -> 8;
field_index(collateral_request, cl_ord_id) -> 9;
field_index(collateral_request, order_id) -> 10;
field_index(collateral_request, secondary_order_id) -> 11;
field_index(collateral_request, secondary_cl_ord_id) -> 12;
field_index(collateral_request, settl_date) -> 13;
field_index(collateral_request, quantity) -> 14;
field_index(collateral_request, qty_type) -> 15;
field_index(collateral_request, currency) -> 16;
field_index(collateral_request, margin_excess) -> 17;
field_index(collateral_request, total_net_value) -> 18;
field_index(collateral_request, cash_outstanding) -> 19;
field_index(collateral_request, side) -> 20;
field_index(collateral_request, price) -> 21;
field_index(collateral_request, price_type) -> 22;
field_index(collateral_request, accrued_interest_amt) -> 23;
field_index(collateral_request, end_accrued_interest_amt) -> 24;
field_index(collateral_request, start_cash) -> 25;
field_index(collateral_request, end_cash) -> 26;
field_index(collateral_request, trading_session_id) -> 27;
field_index(collateral_request, trading_session_sub_id) -> 28;
field_index(collateral_request, settl_sess_id) -> 29;
field_index(collateral_request, settl_sess_sub_id) -> 30;
field_index(collateral_request, clearing_business_date) -> 31;
field_index(collateral_request, text) -> 32;
field_index(collateral_request, encoded_text) -> 33;
field_index(collateral_assignment, sending_time) -> 2;
field_index(collateral_assignment, coll_asgn_id) -> 3;
field_index(collateral_assignment, coll_req_id) -> 4;
field_index(collateral_assignment, coll_asgn_reason) -> 5;
field_index(collateral_assignment, coll_asgn_trans_type) -> 6;
field_index(collateral_assignment, coll_asgn_ref_id) -> 7;
field_index(collateral_assignment, transact_time) -> 8;
field_index(collateral_assignment, expire_time) -> 9;
field_index(collateral_assignment, account) -> 10;
field_index(collateral_assignment, account_type) -> 11;
field_index(collateral_assignment, cl_ord_id) -> 12;
field_index(collateral_assignment, order_id) -> 13;
field_index(collateral_assignment, secondary_order_id) -> 14;
field_index(collateral_assignment, secondary_cl_ord_id) -> 15;
field_index(collateral_assignment, settl_date) -> 16;
field_index(collateral_assignment, quantity) -> 17;
field_index(collateral_assignment, qty_type) -> 18;
field_index(collateral_assignment, currency) -> 19;
field_index(collateral_assignment, margin_excess) -> 20;
field_index(collateral_assignment, total_net_value) -> 21;
field_index(collateral_assignment, cash_outstanding) -> 22;
field_index(collateral_assignment, side) -> 23;
field_index(collateral_assignment, price) -> 24;
field_index(collateral_assignment, price_type) -> 25;
field_index(collateral_assignment, accrued_interest_amt) -> 26;
field_index(collateral_assignment, end_accrued_interest_amt) -> 27;
field_index(collateral_assignment, start_cash) -> 28;
field_index(collateral_assignment, end_cash) -> 29;
field_index(collateral_assignment, trading_session_id) -> 30;
field_index(collateral_assignment, trading_session_sub_id) -> 31;
field_index(collateral_assignment, settl_sess_id) -> 32;
field_index(collateral_assignment, settl_sess_sub_id) -> 33;
field_index(collateral_assignment, clearing_business_date) -> 34;
field_index(collateral_assignment, text) -> 35;
field_index(collateral_assignment, encoded_text) -> 36;
field_index(collateral_response, sending_time) -> 2;
field_index(collateral_response, coll_resp_id) -> 3;
field_index(collateral_response, coll_asgn_id) -> 4;
field_index(collateral_response, coll_req_id) -> 5;
field_index(collateral_response, coll_asgn_reason) -> 6;
field_index(collateral_response, coll_asgn_trans_type) -> 7;
field_index(collateral_response, coll_asgn_resp_type) -> 8;
field_index(collateral_response, coll_asgn_reject_reason) -> 9;
field_index(collateral_response, transact_time) -> 10;
field_index(collateral_response, account) -> 11;
field_index(collateral_response, account_type) -> 12;
field_index(collateral_response, cl_ord_id) -> 13;
field_index(collateral_response, order_id) -> 14;
field_index(collateral_response, secondary_order_id) -> 15;
field_index(collateral_response, secondary_cl_ord_id) -> 16;
field_index(collateral_response, settl_date) -> 17;
field_index(collateral_response, quantity) -> 18;
field_index(collateral_response, qty_type) -> 19;
field_index(collateral_response, currency) -> 20;
field_index(collateral_response, margin_excess) -> 21;
field_index(collateral_response, total_net_value) -> 22;
field_index(collateral_response, cash_outstanding) -> 23;
field_index(collateral_response, side) -> 24;
field_index(collateral_response, price) -> 25;
field_index(collateral_response, price_type) -> 26;
field_index(collateral_response, accrued_interest_amt) -> 27;
field_index(collateral_response, end_accrued_interest_amt) -> 28;
field_index(collateral_response, start_cash) -> 29;
field_index(collateral_response, end_cash) -> 30;
field_index(collateral_response, text) -> 31;
field_index(collateral_response, encoded_text) -> 32;
field_index(collateral_report, sending_time) -> 2;
field_index(collateral_report, coll_rpt_id) -> 3;
field_index(collateral_report, coll_inquiry_id) -> 4;
field_index(collateral_report, coll_status) -> 5;
field_index(collateral_report, tot_num_reports) -> 6;
field_index(collateral_report, last_rpt_requested) -> 7;
field_index(collateral_report, account) -> 8;
field_index(collateral_report, account_type) -> 9;
field_index(collateral_report, cl_ord_id) -> 10;
field_index(collateral_report, order_id) -> 11;
field_index(collateral_report, secondary_order_id) -> 12;
field_index(collateral_report, secondary_cl_ord_id) -> 13;
field_index(collateral_report, settl_date) -> 14;
field_index(collateral_report, quantity) -> 15;
field_index(collateral_report, qty_type) -> 16;
field_index(collateral_report, currency) -> 17;
field_index(collateral_report, margin_excess) -> 18;
field_index(collateral_report, total_net_value) -> 19;
field_index(collateral_report, cash_outstanding) -> 20;
field_index(collateral_report, side) -> 21;
field_index(collateral_report, price) -> 22;
field_index(collateral_report, price_type) -> 23;
field_index(collateral_report, accrued_interest_amt) -> 24;
field_index(collateral_report, end_accrued_interest_amt) -> 25;
field_index(collateral_report, start_cash) -> 26;
field_index(collateral_report, end_cash) -> 27;
field_index(collateral_report, trading_session_id) -> 28;
field_index(collateral_report, trading_session_sub_id) -> 29;
field_index(collateral_report, settl_sess_id) -> 30;
field_index(collateral_report, settl_sess_sub_id) -> 31;
field_index(collateral_report, clearing_business_date) -> 32;
field_index(collateral_report, text) -> 33;
field_index(collateral_report, encoded_text) -> 34;
field_index(collateral_inquiry, sending_time) -> 2;
field_index(collateral_inquiry, coll_inquiry_id) -> 3;
field_index(collateral_inquiry, subscription_request_type) -> 4;
field_index(collateral_inquiry, response_transport_type) -> 5;
field_index(collateral_inquiry, response_destination) -> 6;
field_index(collateral_inquiry, account) -> 7;
field_index(collateral_inquiry, account_type) -> 8;
field_index(collateral_inquiry, cl_ord_id) -> 9;
field_index(collateral_inquiry, order_id) -> 10;
field_index(collateral_inquiry, secondary_order_id) -> 11;
field_index(collateral_inquiry, secondary_cl_ord_id) -> 12;
field_index(collateral_inquiry, settl_date) -> 13;
field_index(collateral_inquiry, quantity) -> 14;
field_index(collateral_inquiry, qty_type) -> 15;
field_index(collateral_inquiry, currency) -> 16;
field_index(collateral_inquiry, margin_excess) -> 17;
field_index(collateral_inquiry, total_net_value) -> 18;
field_index(collateral_inquiry, cash_outstanding) -> 19;
field_index(collateral_inquiry, side) -> 20;
field_index(collateral_inquiry, price) -> 21;
field_index(collateral_inquiry, price_type) -> 22;
field_index(collateral_inquiry, accrued_interest_amt) -> 23;
field_index(collateral_inquiry, end_accrued_interest_amt) -> 24;
field_index(collateral_inquiry, start_cash) -> 25;
field_index(collateral_inquiry, end_cash) -> 26;
field_index(collateral_inquiry, trading_session_id) -> 27;
field_index(collateral_inquiry, trading_session_sub_id) -> 28;
field_index(collateral_inquiry, settl_sess_id) -> 29;
field_index(collateral_inquiry, settl_sess_sub_id) -> 30;
field_index(collateral_inquiry, clearing_business_date) -> 31;
field_index(collateral_inquiry, text) -> 32;
field_index(collateral_inquiry, encoded_text) -> 33;
field_index(network_counterparty_system_status_request, sending_time) -> 2;
field_index(network_counterparty_system_status_request, network_request_type) -> 3;
field_index(network_counterparty_system_status_request, network_request_id) -> 4;
field_index(network_counterparty_system_status_response, sending_time) -> 2;
field_index(network_counterparty_system_status_response, network_status_response_type) -> 3;
field_index(network_counterparty_system_status_response, network_request_id) -> 4;
field_index(network_counterparty_system_status_response, network_response_id) -> 5;
field_index(network_counterparty_system_status_response, last_network_response_id) -> 6;
field_index(user_request, sending_time) -> 2;
field_index(user_request, user_request_id) -> 3;
field_index(user_request, user_request_type) -> 4;
field_index(user_request, username) -> 5;
field_index(user_request, password) -> 6;
field_index(user_request, new_password) -> 7;
field_index(user_request, raw_data) -> 8;
field_index(user_response, sending_time) -> 2;
field_index(user_response, user_request_id) -> 3;
field_index(user_response, username) -> 4;
field_index(user_response, user_status) -> 5;
field_index(user_response, user_status_text) -> 6;
field_index(collateral_inquiry_ack, sending_time) -> 2;
field_index(collateral_inquiry_ack, coll_inquiry_id) -> 3;
field_index(collateral_inquiry_ack, coll_inquiry_status) -> 4;
field_index(collateral_inquiry_ack, coll_inquiry_result) -> 5;
field_index(collateral_inquiry_ack, tot_num_reports) -> 6;
field_index(collateral_inquiry_ack, account) -> 7;
field_index(collateral_inquiry_ack, account_type) -> 8;
field_index(collateral_inquiry_ack, cl_ord_id) -> 9;
field_index(collateral_inquiry_ack, order_id) -> 10;
field_index(collateral_inquiry_ack, secondary_order_id) -> 11;
field_index(collateral_inquiry_ack, secondary_cl_ord_id) -> 12;
field_index(collateral_inquiry_ack, settl_date) -> 13;
field_index(collateral_inquiry_ack, quantity) -> 14;
field_index(collateral_inquiry_ack, qty_type) -> 15;
field_index(collateral_inquiry_ack, currency) -> 16;
field_index(collateral_inquiry_ack, trading_session_id) -> 17;
field_index(collateral_inquiry_ack, trading_session_sub_id) -> 18;
field_index(collateral_inquiry_ack, settl_sess_id) -> 19;
field_index(collateral_inquiry_ack, settl_sess_sub_id) -> 20;
field_index(collateral_inquiry_ack, clearing_business_date) -> 21;
field_index(collateral_inquiry_ack, response_transport_type) -> 22;
field_index(collateral_inquiry_ack, response_destination) -> 23;
field_index(collateral_inquiry_ack, text) -> 24;
field_index(collateral_inquiry_ack, encoded_text) -> 25;
field_index(confirmation_request, sending_time) -> 2;
field_index(confirmation_request, confirm_req_id) -> 3;
field_index(confirmation_request, confirm_type) -> 4;
field_index(confirmation_request, alloc_id) -> 5;
field_index(confirmation_request, secondary_alloc_id) -> 6;
field_index(confirmation_request, individual_alloc_id) -> 7;
field_index(confirmation_request, transact_time) -> 8;
field_index(confirmation_request, alloc_account) -> 9;
field_index(confirmation_request, alloc_acct_id_source) -> 10;
field_index(confirmation_request, alloc_account_type) -> 11;
field_index(confirmation_request, text) -> 12;
...
field_by_number(<<"10130">>) -> tag10130;
field_by_number(<<"10455">>) -> tag10455;
...
field_by_number(<<"35002">>) -> cancel_on_disconnect_type;
...
decode_typed_field(exec_type, <<"1">>) -> 'partialfill';
...
decode_typed_field(exec_type, <<"R">>) -> 'received';
...
decode_typed_field(pos_type, <<"ALC">>) -> 'allocationtradeqty';
decode_typed_field(pos_type, <<"AS">>) -> 'optionassignment';
decode_typed_field(pos_type, <<"ASF">>) -> 'asoftradeqty';
decode_typed_field(pos_type, <<"DLV">>) -> 'deliveryqty';
decode_typed_field(pos_type, <<"ETR">>) -> 'electronictradeqty';
decode_typed_field(pos_type, <<"EX">>) -> 'optionexerciseqty';
decode_typed_field(pos_type, <<"FIN">>) -> 'endofdayqty';
decode_typed_field(pos_type, <<"IAS">>) -> 'intraspreadqty';
decode_typed_field(pos_type, <<"IES">>) -> 'interspreadqty';
decode_typed_field(pos_type, <<"PA">>) -> 'adjustmentqty';
decode_typed_field(pos_type, <<"PIT">>) -> 'pittradeqty';
decode_typed_field(pos_type, <<"SOD">>) -> 'startofdayqty';
decode_typed_field(pos_type, <<"SPL">>) -> 'integralsplit';
decode_typed_field(pos_type, <<"TA">>) -> 'transactionfromassignment';
decode_typed_field(pos_type, <<"TOT">>) -> 'totaltransactionqty';
decode_typed_field(pos_type, <<"TQ">>) -> 'transactionquantity';
decode_typed_field(pos_type, <<"TRF">>) -> 'transfertradeqty';
decode_typed_field(pos_type, <<"TX">>) -> 'transactionfromexercise';
decode_typed_field(pos_type, <<"XM">>) -> 'crossmarginqty';
...
decode_typed_field(security_trading_statusb, V) -> V;
decode_typed_field(security_trading_statusc, V) -> V;
decode_typed_field(protection_price, V) -> parse_num(V)*1.0;
decode_typed_field(bts_final_tx_ord_status, <<"0">>) -> 'new';
decode_typed_field(bts_final_tx_ord_status, <<"1">>) -> 'partialfilled';
decode_typed_field(bts_final_tx_ord_status, <<"2">>) -> 'filled';
decode_typed_field(bts_final_tx_ord_status, <<"4">>) -> 'canceled';
decode_typed_field(bts_final_tx_ord_status, <<"5">>) -> 'replaced';
decode_typed_field(bts_final_tx_ord_status, <<"8">>) -> 'rejected';
decode_typed_field(poss_missing_appl_msg, V) -> V == <<"Y">>;
decode_typed_field(tag10100, V) -> V;
decode_typed_field(tag10101, V) -> V;
decode_typed_field(tag10121, V) -> V;
decode_typed_field(tag10130, V) -> V;
decode_typed_field(tag10455, V) -> V;
decode_typed_field(tag10645, V) -> V == <<"Y">>;
decode_typed_field(tag10702, V) -> V;
decode_typed_field(tag10703, V) -> V;
decode_typed_field(cancel_on_disconnect_type, V) -> parse_num(V);
...
encode_typed_field(exec_type, 'partialfill') -> <<"1">>;
...
encode_typed_field(exec_type, 'received') -> <<"R">>;
...
encode_typed_field(bts_final_tx_ord_status, 'filled') -> <<"2">>;
encode_typed_field(bts_final_tx_ord_status, 'canceled') -> <<"4">>;
encode_typed_field(bts_final_tx_ord_status, 'replaced') -> <<"5">>;
...
encode_typed_field(cancel_on_disconnect_type, V) when is_integer(V) -> list_to_binary(integer_to_list(V));
...
encode_typed_field(pos_type, 'deliveryqty') -> <<"DLV">>;
encode_typed_field(pos_type, 'electronictradeqty') -> <<"ETR">>;
encode_typed_field(pos_type, 'optionexerciseqty') -> <<"EX">>;
encode_typed_field(pos_type, 'endofdayqty') -> <<"FIN">>;
encode_typed_field(pos_type, 'intraspreadqty') -> <<"IAS">>;
encode_typed_field(pos_type, 'interspreadqty') -> <<"IES">>;
encode_typed_field(pos_type, 'adjustmentqty')-> <<"PA">>;
encode_typed_field(pos_type, 'pittradeqty') -> <<"PIT">>;
encode_typed_field(pos_type, 'startofdayqty') -> <<"SOD">>;
encode_typed_field(pos_type, 'integralsplit') -> <<"SPL">>;
encode_typed_field(pos_type, 'transactionfromassignment') -> <<"TA">>;
encode_typed_field(pos_type, 'totaltransactionqty') -> <<"TOT">>;
encode_typed_field(pos_type, 'transactionquantity') -> <<"TQ">>;
encode_typed_field(pos_type, 'transfertradeqty') -> <<"TRF">>;
encode_typed_field(pos_type, 'transactionfromexercise') -> <<"TX">>;
encode_typed_field(pos_type, 'crossmarginqty') -> <<"XM">>;
...
number_by_field(tag10130) -> <<"10130">>;
number_by_field(tag10455) -> <<"10455">>;
...
number_by_field(cancel_on_disconnect_type) -> <<"35002">>;
