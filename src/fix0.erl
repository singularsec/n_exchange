...
record_to_proplist(#quote_request{} = Rec) ->
...
crack(#order_cancel_replace_request{} = NOS) ->
    Prop = record_to_proplist(NOS),
    ?DBG("order_cancel_replace_request ~n ~p ~n", [Prop]);
...
decode0(Bin) ->
  case decode_fields(Bin) of
    {ok, Fields, MessageBin, Rest} ->
      ...
      {ok, fix_parser44_xp:decode_message(Fields), MessageBin, Rest};
    Else ->
      Else
  end.
    ...
    [BinLen, Rest1] ->
      ...
      case Rest1 of
        <<Message:BodyLength/binary, "10=", _CheckSum:3/binary, 1, Rest2/binary>> ->
          ...
          {ok, fix_splitter44_xp:split(Message), MessageBin, Rest2};
        _ ->
...
encode([{_K,_V}|_] = Packet) ->
  [[fix_parser44_xp:number_by_field(Key), "=", fix_parser44_xp:encode_typed_field(Key, Value), 1] || {Key, Value} <- Packet].
